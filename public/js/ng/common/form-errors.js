  ...
  .directive('uiValidationErrorMessages', ['ErrorMessages', '$parse',
    function (ErrorMessages, $parse) {
      ...
      return {
        ...
        ' <small ng-repeat="(errorType, errorValue) in fieldCtrl.$error" ng-show="fieldCtrl.$error[errorType]">{{errorMessages[errorType]}} <span ng-show="errorType != \'pattern\'" ng-bind="{{errorType}}"></span></small>' +
        '</div>',
        scope: {},
